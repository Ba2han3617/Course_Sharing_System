...
namespace CourseNoteSharingSystem.Controllers
{
    ...
    public class NotesController : BaseController
    {
        ...
        {
            if (!IsAuthenticated) return Redirect("/Auth/Login?returnUrl=/Notes/Upload");
        ...
        public async Task<IActionResult> Upload(UploadNoteDto dto)
        {
            ...
            var (success, error, noteId) = await _noteService.UploadNoteAsync(dto, CurrentUserId!.Value);
            if (!success)
            {
        ...
        public async Task<IActionResult> Edit(EditNoteDto dto)
        {
            ...
            var (success, error) = await _noteService.EditNoteAsync(dto, CurrentUserId!.Value);
            if (!success)
            {
                ModelState.AddModelError("", error!);
                ViewBag.Courses = await _courseService.GetAllAsync();
        ...
        {
            if (!IsAuthenticated) return Redirect("/Auth/Login");
            var (success, error) = await _noteService.DeleteNoteAsync(id, CurrentUserId!.Value, IsAdmin);
